...
export function LineChart({ title, subtitle, height, series, xKey, yKeys, colors, showLegend = true, className }: XYChartProps) {
  ...
  return (
    <ChartContainer title={title} subtitle={subtitle} height={height} className={className}>
      <ResponsiveContainer width="100%" height="100%">
        <RLineChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
          ...
          {(yKeys || series.map(s => s.name)).map((key, i) => (
            <Line key={String(key)} type="monotone" dataKey={String(key)} stroke={palette[i % (palette.length || 1)] || '#3B82F6'} strokeWidth={2} />
          ))}
        </RLineChart>
      </ResponsiveContainer>
    </ChartContainer>
