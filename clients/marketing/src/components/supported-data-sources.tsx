        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
            {filteredDataSources.map((source) => (
              ...
              >
                <div className="w-16 h-16 flex items-center justify-center bg-input border border-border rounded-xl group-hover:bg-gray-900/80 group-hover:border-gray-700 transition-all duration-300">
                  <Image 
                    src={source.logo} 
                    alt={source.name} 
                    width={40} 
          ...
          {showSearch && filteredDataSources.length === 0 && (
            <div className="text-center mt-12">
              <p className="text-muted-foreground">No data sources found matching "{searchTerm}"</p>
              <p className="text-gray-500 text-sm mt-2">Don't see your data source? <span className="text-foreground hover:text-muted-foreground cursor-pointer">Contact us</span> and we'll add it.</p>
            </div>
          )}
          
          {showSearch && filteredDataSources.length > 0 && searchTerm && (
            <div className="text-center mt-12">
              <p className="text-muted-foreground">Found {filteredDataSources.length} data source{filteredDataSources.length !== 1 ? 's' : ''}</p>
            </div>
          )}
        </div>
      </section>
