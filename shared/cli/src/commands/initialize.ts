...
export const initializeCommand = async () => {
  ...
  if (fs.existsSync(path.join(cwd, 'serverless.ts'))) {
    if (fs.existsSync(path.join(cwd, 'config.zero'))) {
      console.log(
        chalk.yellow(
          'This directory already contains a zeiro service with configuration.',
        ),
      )
      process.exit(0)
    } else {
      console.log(
        chalk.yellow(
          'Detected an existing service without zeiro configuration.',
        ),
      )
...
async function configureExistingService(cwd: string) {
  ...
  const config: ServiceConfig = {
    ...
    environment: {
      ...
      API_URL: `/zeiro/\${stage}/infrastructure/io/central/api/url`,
    },
  }

  await writeFileAsync(
    path.join(cwd, 'config.zero'),
    JSON.stringify(config, null, 2),
  )

  console.log(chalk.green(`✓ Service configuration saved to config.zero`))
}
...
async function initializeService(cwd: string) {
  ...
  try {
    ...
    await writeFileAsync(
      path.join(servicePath, 'config.zero'),
      JSON.stringify(config, null, 2),
    )
...
async function createServiceFiles(
  ...
  // Create package.json
  const packageJson = {
    name: `@zeiro/${layer === 'domain' && domain ? `${domain}-` : ''}${serviceName}-service`,
    version: '0.9.0',
    ...
    scripts: {
      ...
      'generate-env': 'zeiro env',
      test: 'jest --silent',
      'test:silent': 'zeiro env && jest --silent',
    },
    dependencies: {
      '@zeiro/data-infrastructure': '*',
      '@zeiro/platform-infrastructure': '*',
      inversify: '^6.0.1',
  ...
  const tsconfig = {
    compilerOptions: {
      ...
      baseUrl: 'application',
      paths: {
        '@zeiro/sdk': ['../../../../../shared/typescript/sdk/src/index.ts'],
        '@adapters/*': ['adapters/*'],
        '@domain/*': ['domain/*'],
...
`
  ...
  const serverlessTs = `import { AWS } from '@zeiro/sdk'

/** Serverless configuration for the '${serviceName}' service. */
const serverlessConfiguration: AWS.Service = {
  service: 'zeiro-${layer === 'domain' && domain ? `${domain}-` : ''}${serviceName}',
  frameworkVersion: '3',
...
`
  ...
  const createEntityDefinitionContent = `import { AWS, handlerPath } from '@zeiro/sdk'
...
export const definition: AWS.ServerlessLambdaFunction = {
  ...
  iamRoleStatements: [
    {
      ...
      Resource: [
        '\${ssm:/zeiro/\${self:custom.stage}/domain/${serviceName}/infrastructure/storage/table/arn}',
      ],
    },
    {
      Effect: 'Allow',
  ...
  environment: {
    TABLE_NAME:
      '\${ssm:/zeiro/\${self:custom.stage}/domain/${serviceName}/infrastructure/storage/table/name}',
  },
}
`
  await writeFileAsync(
...
`
  ...
  // models/entity.ts
  const entityModelContent = `import { Resource } from '@zeiro/sdk'
...
`
  ...
  const eventsTypesContent = `import { DomainCommand } from '@zeiro/sdk'
...
import { DomainCommand, DomainEvent } from '@zeiro/sdk'
...
`
  ...
  // domain-errors.ts
  const errorsContent = `import { DomainError } from '@zeiro/sdk'
